...
def page(context, request):
    ...
    if request.node.rep_call.failed:
        page.screenshot(
            path=f"artifacts/screenshots/{request.node.name}.png", full_page=True
        )
    page.close()
    if request.node.rep_call.failed:
...
def artifacts(request):
    ...
    if request.node.rep_call.failed:
        allure.attach.file(
            f"artifacts/screenshots/{request.node.name}.png",
            name="screenshot",
            attachment_type=allure.attachment_type.PNG,
        )
        allure.attach.file(
            f"artifacts/videos/{request.node.name}.webm",
            name="video",
...
def navigate_to_dpf_page(request):
    language = request.param
...
def navigate_to_nsf_page(request):
    language = request.param
...
def delete_user_after_test(fake_email):
    ...
    data = {"emails": [fake_email]}
    response = requests.post(DELETE_USER_URL, headers=headers, json=data)
    if response.status_code == 200:
        pass
    else:
        print(
            f"Failed to delete user with email {fake_email}. Status code: {response.status_code}"
        )
