...
from config import qr_create_methods
...
class TestFrameComparatives(BaseTest):
    ...
    def test_comparative_preview_and_view(
        self, sign_up_fixture, browser, request, qr_create_method
    ):
        test_func = request.node.originalname
        expected_image_path = (
            f"artifacts/snapshots/preview/Expected/{test_func}"
            f"[{browser.browser_type.name}-{qr_create_method}-{browser.browser_type.name}].png"
        )
        actual_image_path = (
            f"artifacts/snapshots/preview/Actual/{test_func}"
            f"[{browser.browser_type.name}-{qr_create_method}-{browser.browser_type.name}].png"
        )
        diff_image_path = (
            f"artifacts/snapshots/preview/Diff/{test_func}"
            f"[{browser.browser_type.name}-{qr_create_method}-{browser.browser_type.name}].png"
        )
        ...
        self.my_qr_codes_page.expect(
            self.my_qr_codes_page.locator.header_create_qr_code_button
        ).to_be_visible()
        new_page = self.my_qr_codes_page.open_in_new_tab(
            self.my_qr_codes_page.open_last_qr_link()
        )
        ...
        allure_attach_image_diff(
            actual_image_path, expected_image_path, diff_image_path
        )
