...
class TestNSFSignUpFlowJpeg(BaseTest):
    ...
    @allure.description("Test QR code creation through sign up flow with different creation methods and resolutions")
    ...
    def test_nsf_sign_up_qr_type(
        ...
        with allure.step(f"Download QR code in JPEG format with {resolution} resolution"):
            self.my_qr_codes_page.download_parametrize_files(
                jpeg_download_format, resolution
            )

    @allure.severity(allure.severity_level.CRITICAL)
    @allure.title("Create and download website QR code with {qr_create_method} method and {resolution} resolution")
    @allure.description("Test website QR code creation through sign up flow with specific methods and resolutions")
